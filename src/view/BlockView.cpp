...
BlockView::BlockView(std::shared_ptr<GameController> controller, int color, int x, int y, QWidget* parent): QWidget(parent),
...
void BlockView::mousePressEvent(QMouseEvent* event) {
    ...
    qDebug() << "BlockView::mousePressEvent" << " - play at : " << x << "," << y;
}
